...

{
  ...
  //convert srgb_color_image to HSL_color_image
  FImage *HSL_color_image = convert_srgb2HSL(srgb_color_image);

  //convert HSL_color_image to srgb_color_image2
  FImage *srgb_color_image2 = convert_HSL2srgb(HSL_color_image);
  //print srgb_color_image2
  PImage *Pimage3 = srgb_color_image2->toPImage();
  Pimage3->write((char *)"srgb_color_image2.png");

  FImage *HSL_color_image_new = new FImage(w, h);

  for (int y = 0; y < h; y++) {
    for (int x = 0; x < w; x++) {

      float hsl3[3];
      HSL_color_image->get(x, y, hsl3);
      ...
      //compute the new saturation (the S in HSL)
      C = hsl3[1];
      Cp = C * ( 1.0 - ( 1.0 - C ) * ( d - 1.0 ) );
      Cp = C;
      hsl3_new[1] = Cp;

      //compute the new luminance (the L in HSL)
      C = hsl3[2];
      Cp = C * ( 1.0 - ( 1.0 - C ) * ( d - 1.0 ) );
      //Cp = C;
      hsl3_new[2] = Cp;

      HSL_color_image_new->set(x, y, hsl3_new);
    }
  }

  //convert HSL_color_image_new to srgb_color_image_new
  FImage *srgb_color_image_new = convert_HSL2srgb(HSL_color_image_new);
